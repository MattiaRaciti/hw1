...
function search(event){
	...
	if(content) {
		...
		if(tipo == "image") {
			...
			fetch("http://localhost/hw1/bing_api.php?param=" + text).then(onResponse).then(onJson);
		}
		...
		if(tipo == "user") {
			...
			fetch("http://localhost/hw1/user_search.php?param=" + text + "%").then(onResponseUser).then(onJsonUser);
		}
...
function seleziona(event){
	...
	fetch("http://localhost/hw1/add_favorite.php?param=" + urls[indiceDaSegnareDatabase]).then(onResponseFavorite).then(onFavorite);
	...
	if(json.success == true){
		for(const element of json.content){
			...
			var username_text = document.createTextNode("UTENTE: " + username + "\n" +
														"ISCRITTO DAL: " + since.substring(0, 10));
...
function visualizzaCollezione(){
	...
	fetch("http://localhost/hw1/get_collection.php?param=" + username).then(onResponseFavoriteUser).then(onFavoriteUser);
}
...
function sendLike(){
	...
	fetch("http://localhost/hw1/add_like.php?param=" + username).then(onResponseLike);
...
function undoLike(){
	...
	fetch("http://localhost/hw1/undo_like.php?param=" + username).then(onResponseUndoLike);
}
